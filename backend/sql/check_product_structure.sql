-- =====================================================
-- Script kiểm tra cấu tạo sản phẩm trong database
-- =====================================================
-- HƯỚNG DẪN: Thay thế số 14 bằng project_id bạn muốn kiểm tra
-- Hoặc xóa dòng WHERE để xem tất cả dự án

-- 1. Kiểm tra các sản phẩm trong project_items
SELECT 
    ...
    pi.snapshot_config IS NOT NULL as has_snapshot,
    pt.code as template_code,
    pt.name as template_name,
    pt.product_type as door_type,
    pt.structure_json IS NOT NULL as has_structure_json,
...
WHERE pi.project_id = 14  -- ⚠️ THAY SỐ 14 BẰNG project_id CẦN KIỂM TRA
ORDER BY pi.project_id, pi.id;

-- 2. Kiểm tra door_designs đã được tạo chưa
SELECT 
...
WHERE dd.project_id = 14  -- ⚠️ THAY SỐ 14 BẰNG project_id CẦN KIỂM TRA
ORDER BY dd.project_id, dd.id;

-- 3. Kiểm tra BOM đã được bóc tách chưa
SELECT 
    bi.design_id,
    dd.project_item_id,
...
-- 4. Tổng hợp: Kiểm tra tất cả sản phẩm trong một project
SELECT 
    pi.id as project_item_id,
    p.project_name,
    pi.quantity,
